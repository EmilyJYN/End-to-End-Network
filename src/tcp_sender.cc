...
uint64_t TCPSender::consecutive_retransmissions() const
{
  ...
  /* Send a TCPSenderMessage from message_out_ if needed (or empty optional otherwise) */
optional<TCPSenderMessage> TCPSender::maybe_send()
{
  // Your code here.
  if ( messages_out_.empty() ) { return nullopt; }

  TCPSenderMessage msg = messages_out_.front(); //queue, FIFO
  messages_out_.pop();
  return msg;
}

  /* Push bytes from the app (outbound stream) to a queue (message_out_), and */
...
void TCPSender::tick( uint64_t ms_since_last_tick )
{
  ...
  while ( tick_countdown >= timer_ ) {
    // Update tick countdown and retx count.
    tick_countdown -= timer_;
