...
optional<InternetDatagram> NetworkInterface::recv_frame( const EthernetFrame& frame )
{
  ...
  if ( frame.header.type == EthernetHeader::TYPE_IPv4 ) { // Payload is IPv4.
    InternetDatagram dgram;
    if ( parse( dgram, frame.payload ) ) {
      // If no error, return dgram.
      return dgram;
    }
  } else if ( frame.header.type == EthernetHeader::TYPE_ARP ) { // Payload is ARP.
    ARPMessage arp;
    if ( parse( arp, frame.payload ) ) {

      // whatever it is, save its IP:Ethernet mapping first
      ...
      if(arp.target_ip_address == ip_address_.ipv4_numeric()){
        ...
        if ( arp.opcode == ARPMessage::OPCODE_REPLY ) {
          ...
          // if host is waiting for Ethernet address of this IP, send and clean datagram waitlist
          if ( waitlist_it != arp_waitlist_.end() ) {
            ...
            arp_waitlist_.erase( waitlist_it );
          }
        }
      }
    }
...
{
  //update TTL for each cached mapping
  ...
  // update time waited for each datagram in wait-list 
